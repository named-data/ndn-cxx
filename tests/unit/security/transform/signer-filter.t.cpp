...
BOOST_AUTO_TEST_CASE(Rfc4231Test1)
{
  ...
  const uint8_t hmacSm3[] = {0x51, 0xb0, 0x0d, 0x1f, 0xb4, 0x98, 0x32, 0xbf, 0xb0, 0x1c,
							 0x3c, 0xe2, 0x78, 0x48, 0xe5, 0x9f, 0x87, 0x1d, 0x9b, 0xa9, 
							 0x38, 0xdc, 0x56, 0x3b, 0x33, 0x8c, 0xa9, 0x64, 0x75, 0x5c, 
							 0xce, 0x70};
#endif
  ...
  OBufferStream os224;
  bufferSource(data) >> signerFilter(DigestAlgorithm::SHA224, key, keyType) >> streamSink(os224);
  auto sig = os224.buf();
  ...
  OBufferStream os384;
  bufferSource(data) >> signerFilter(DigestAlgorithm::SHA384, key, keyType) >> streamSink(os384);
  sig = os384.buf();
