...
#if 1
const uint32_t SM2_KEY_SIZES[] = {224, 256, 384, 521};
const uint32_t DEFAULT_SM2_KEY_SIZE = 256;
#endif
...
#if 1
uint32_t
sm2KeyParamsInfo::checkKeySize(uint32_t size)
{
  for (size_t i = 0; i < (sizeof(SM2_KEY_SIZES) / sizeof(SM2_KEY_SIZES[0])); i++) {
    if (SM2_KEY_SIZES[i] == size)
      return size;
  }
  NDN_THROW(KeyParams::Error("Unsupported sm2 ec key size " + to_string(size)));
}

uint32_t
sm2KeyParamsInfo::getDefaultSize()
{
  return DEFAULT_SM2_KEY_SIZE;
}
