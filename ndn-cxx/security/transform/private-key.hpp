...
public:
  ...
  //added_GM, added_HMAC by liupenghui
  //if the PrivateKey is imported from outside, detail::getEvpPkeyType(m_impl->key)) can't differ SM2 from ECDSA, thus, we add a parameter keyType.
	...
	[[deprecated("use the overload that takes a span<>")]]
	ConstBufferPtr
	decrypt(const uint8_t* cipherText, size_t cipherLen, KeyType keyType) const
	{
	  return decrypt({cipherText, cipherLen}, keyType);
	}
	
	/**
	 * @brief Load the HMAC private key in PKCS#1 format from a stream @p is
	 */
	void
	loadHamcPkcs1Base64(std::istream& is);
...
#if 1
  ConstBufferPtr
  sm2Decrypt(span<const uint8_t> cipherText) const;
#endif
...
#if 1
  static unique_ptr<PrivateKey>
  generateSM2Key(uint32_t keySize);
#endif
